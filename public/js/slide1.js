var app = angular.module('myapp', []).constant('api', '/api/slide1s/');
...
function mycontrol($scope, $http, $filter, api) {
    ...
    $http({
        ...
        $scope.slide1s = res.data;
    });
    $scope.numberOfPages = function() {
        return Math.ceil($scope.slide1s.length / $scope.pageSize);
    }
    $scope.getData = function() {

        return $filter('filter')($scope.slide1s, $scope.timkiem)
    }
