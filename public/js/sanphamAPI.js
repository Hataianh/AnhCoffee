var app = angular.module('myapp', []).constant('api', 'https://localhost:44370/api/SanPham');
...
function mycontrol($scope, $http, $filter, api) {
    ...
    $scope.showModal = function(id) {
        ...
        fileUpload.addEventListener("change", (e) => {
            ...
            document.getElementById('hinh_anh').innerHTML = `<img src="/admin_layout/uploads/` + files[0].name + `" alt="Ảnh" style="width:100%;height:100%">`;
            for (const file of files) {
                const { name, type, size, lastModified } = file;
                // Làm gì đó với các thông tin trên
                $scope.sanpham.hinh_anh = file.name;
                document.getElementById('hinh_anh').innerHTML = `<img src="/admin_layout/uploads/` + file.name + `" alt="Ảnh" style="width:100%;height:100%">`;
            };
        });
    }
