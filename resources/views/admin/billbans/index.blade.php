...
<div lang="en" ng-app="myapp" ng-controller="mycontrol" class="card">
    ...
    <p><button class="btn btn-success" ng-click="showModal(0)"> Thêm mới <i class="fa fa-plus"></i></button></p>
        ...
        <table class="table table-bordered table-stripper">
            ...
            <tr ng-repeat="bb in staffs|filter: timkiem | begin:currentPage*pageSize | limitTo:pageSize">
        ...
        <td>@{{bb.status}}</td>
        <td>@{{bb.note}}</td>
        <td><button class=" btn btn-primary" ng-click="showModal(bb.id)"> <i class="fa fa-pen"></i></button></td>
                <td><button class="btn btn-danger" ng-click="deleteClick(bb.id)"> <i class="fa fa-trash"></i></button></td>
            </tr>
        </table>
                    ...
                    <div class="modal-body">
                        <div class="container-fluid">
                            <div class="form-group">
                                <label for="">ID KH: </label>
                                <div><input type="number" ng-model="staff.id_kh" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">Ngày order: </label>
                                <div><input type="date" ng-model="staff.date_order" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">Tổng tiền: </label>
                                <div><input type="number" ng-model="staff.tong_tien" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">Thanh toán: </label>
                                <div><input type="text" ng-model="staff.payment" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">Trạng thái: </label>
                                <div><input type="text" ng-model="staff.status" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">Ghi chú: </label>
                                <div><input type="text" ng-model="staff.note" class="form-control"></div>
                            </div>
                        </div>
                    </div>
