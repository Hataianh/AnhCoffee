@extends('_admin_layout')
@section('content')
<div lang="en" ng-app="myapp" ng-controller="mycontrol" class="card">
    <div class="card-header">
        <i class="fas fa-table me-1"></i>
        Danh sách sản phẩm
    </div>
    <div class="card-body">
        <p><button class="btn btn-success" ng-click="showModal(0)"> Thêm mới <i class="fa fa-plus"></i></button></p>

        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="dataTables_length" id="example_length">
                    <label>Show
                        <select ng-model="pageSize" id="pageSize"
                            class="custom-select custom-select-sm form-control form-control-sm">
                            <option value="5">5</option>
                            <option value="10">10</option>
                            <option value="15">15</option>
                            <option value="20">20</option>
                            <option value="25">25</option>
                        </select>
                        entries</label>
                </div>
            </div>
            <div class="col-sm-12 col-md-6">
                <div id="example_filter" class="dataTables_filter"><label>Search:
                        <input type="search" class="form-control form-control-sm" ng-model="timkiem"></label></div>
            </div>
        </div>

        <table class="table table-bordered table-stripper">
            <tr>
                <th>TT</th>
                <th>Tên SP</th>
                <th>Hình ảnh</th>
                <th>Mô tả</th>
                <th>Giá niêm yết</th>
                <th>Giá bán</th>
                <th>Đơn vị tính</th>
                <th>Số lượng</th>
                <th>Sửa</th>
                <th>Xóa</th>

            </tr>
            <tr ng-repeat="sp in sanphams|filter: timkiem | begin:index-1+(currentPage)*pageSize | limitTo:pageSize">
                <td>@{{$index+1+(currentPage)*pageSize}}</td>
                <td>@{{sp.ten_sp}}</td>
                <td><img src="/admin_layout/uploads/@{{sp.hinh_anh}}" style="width: 50px"></td>
                <td>@{{sp.mota_sp}}</td>
                <td>@{{sp.gia_kkm | number:0}}</td>
                <td>@{{sp.gia_km | number:0}}</td>
                <td>@{{sp.don_vi_tinh}}</td>
                <td>@{{sp.so_luong}}</td>
                <td><button class=" btn btn-primary" ng-click="showModal(sp.id)"> <i class="fa fa-pen"></i></button>
                </td>
                <td><button class="btn btn-danger" ng-click="deleteClick(sp.id)"> <i class="fa fa-trash"></i></button>
                </td>
            </tr>
        </table>
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item" ng-disabled="currentPage == 0" ng-click="currentPage=currentPage-1"><a
                        class="page-link" href="#"><i class="fa fa-angle-left"></i></a></li>
                <li class="page-item"><a class="page-link" href="#">@{{currentPage + 1}}/@{{numberOfPages()}}</a></li>
                <li class="page-item" ng-disabled="currentPage >= getData().length/pageSize - 1"
                    ng-click="currentPage=currentPage+1"><a class="page-link" href="#"><i
                            class="fa fa-angle-right"></i></a></li>
            </ul>
        </nav>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js"
            integrity="sha512-894YE6QWD5I59HgZOGReFYm4dnWc1Qt5NtvYSaNcOP+u1T9qYdvdihz0PPSiiqn/+/3e7Jo4EaG7TubfWGUrMQ=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.0.0-alpha.4/js/bootstrap.min.js"
            integrity="sha512-Cy3gSrKCS8aJ7AIaammc0wLXyKRmTa8ntgHvU01Tuz4EdsqVgk/lKzFm/b/8RxOWBaoHI2uGLLU6rXMbqKcGHA=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js"></script>
        <script src="https://cdn.datatables.net/1.11.5/js/jquery.dataTables.min.js"></script>
        <script src="/js/angular.min.js"></script>
        <script src="/js/sanphamAPI.js"></script>

        <script src="https://cdn.ckeditor.com/ckeditor5/34.0.0/classic/ckeditor.js"></script>
        <!-- Modal -->
        <div class="modal fade" id="modelId" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content" style="width: 150%">
                    <div class="modal-header">
                        <h5 class="modal-title">@{{modaltitle}}</h5>
                        <button type="button" ng-click="hideModal()" class="close" data-dismiss="modal"
                            aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <div class="container-fluid">
                            <div class="form-group">
                                <label for="">Tên SP: </label>
                                <div><input type="text" ng-model="sanpham.name" class="form-control"></div>
                            </div>
                            <div class="form-group row">
                                <label for="name">Hình ảnh:</label>
                                <div>
                                    <input type="file" class="form-control" id="file-upload" ng-model="sanpham.image">
                                </div>
                                </br>
                                <div style="width:100px;height:100px" id="image">
                                    <img src="/admin_layout/uploads/@{{sanpham.image}}" alt="Ảnh" style="width:100px;height:100px"
                                        ng-model="sanpham.image">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="">Mô tả: </label>
                                <div><textarea cols="50" rows="5" style="overflow-y: scroll" type="text" ng-model="sanpham.mota_sp" class="form-control"></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="">Giá niêm yết: </label>
                                <div><input type="number" ng-model="sanpham.gia_kkm" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">Giá bán: </label>
                                <div><input type="number" ng-model="sanpham.unit_price" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">Đơn vị tính: </label>
                                <select ng-model="sanpham.don_vi_tinh" class="form-control">
                                    <option value="cốc take away">cốc take away</option>
                                    <option value="gói">gói</option>
                                    <option value="cốc">cốc</option>
                                    <option value="tách">tách</option>
                                    <option value="ly">ly</option>
                                    <option value="lon">lon</option>
                                    <option value="cái">cái</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="">Số lượng: </label>
                                <div><input type="number" ng-model="sanpham.so_luong" class="form-control"></div>
                            </div>
                            <div class="form-group">
                                <label for="">ID loại SP: </label>
                                <select ng-model="sanpham.id_loai_sp" class="form-control">
                                    <option value="1">1</option>
                                    <option value="2">2</option>
                                    <option value="3">3</option>
                                    <option value="4">4</option>
                                    <option value="5">5</option>
                                    <option value="6">6</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="">Tên loại SP: </label>
                                <select ng-model="sanpham.tenloai" class="form-control">
                                    <option value="Coffee">Coffee</option>
                                    <option value="Nước ép">Nước ép</option>
                                    <option value="Trà sữa">Trà sữa</option>
                                    <option value="Cocktail">Cocktail</option>
                                    <option value="Bánh">Bánh</option>
                                    <option value="Trà">Trà</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="">ID NCC: </label>
                                <select ng-model="sanpham.id_ncc" class="form-control">
                                    <option value="1">1</option>
                                    <option value="2">2</option>
                                    <option value="3">3</option>
                                    <option value="4">4</option>
                                    <option value="5">5</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="">New: </label>
                                <select ng-model="sanpham.new" class="form-control">
                                    <option value="Có">Có</option>
                                    <option value="Không">Không</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" ng-click="hideModal()" class="btn btn-secondary"
                            data-dismiss="modal">Close</button>
                        <button type="button" class="btn btn-primary" ng-click="saveData()">Save</button>
                    </div>
                </div>
            </div>
        </div>


        @stop
